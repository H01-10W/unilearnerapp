/*
 * Persists note-specific mastery concepts, stage evidence, and metaphor assets.
 * Concepts are regenerated by stable keys, while practice state is retained
 * separately so content refreshes do not erase learner progress accidentally.
 */
...
function migrateMasteryConceptsSchema(db) {
  ...
  try {
    ...
    try {
      ...
      // Preserve the migration failure itself; a secondary rollback failure
      // cannot explain the schema problem as well as the original error.
    }
    throw error;
  } finally {
...
function deleteDocumentImageAssets(imagePaths) {
  imagePaths.forEach((imagePath) => {
    try {
      ...
      // Asset deletion is deliberately best effort: stale generated files must
      // not prevent the database state from being cleared.
    }
  });
}
